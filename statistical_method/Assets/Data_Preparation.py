...
class Data_Preparation:
    ...
    def __call__(
            ...
            unWanted_columns, Scaling_method,
        ...
        for target_column in target_columns:
            self.Log(f'\n..:: Start Data_Preparation \"{file_name}\" for {target_column} with '+
                     f'(random_state: {random_state}, Scaling_method: {Scaling_method}) ::..')
        
            main_df = pd.read_csv(f'{self.iPath}/{file_name}.csv')
            self.Log(f'==> \"{file_name}\" shape: {main_df.shape}')
            
            tmp_path = f'{self.oPath}/{file_name}/{target_column}/{random_state}_{Scaling_method}'
            ...
            tmp_dropColumns = target_columns+unWanted_columns
            X = main_df.drop(tmp_dropColumns, axis=1)
            y = main_df[target_column]

            for fold_number, (train_index, test_index) in enumerate(skf.split(X, y)):

                # Splitting
                df_train = main_df.iloc[train_index]
                df_test = main_df.iloc[test_index]
                self.Log(
                ...
                train_values = df_train.drop(tmp_dropColumns, axis=1)
                test_values = df_test.drop(tmp_dropColumns, axis=1)

                # Scaling
                copy_train_values = train_values.copy()
                copy_test_values = test_values.copy()

                if "MinMax" in Scaling_method:
                ...
                KBest_finalScores = self.KBest_ranking(
                    df_train, n_features, target_column, tmp_dropColumns,
                    number_of_splits, shuffle_flag, random_state,
            ...
            self.Log(f'..:: End Data_Preparation \"{file_name}\" for {target_column} with '+
                     f'(random_state: {random_state}, Scaling_method: {Scaling_method}) ::..')
