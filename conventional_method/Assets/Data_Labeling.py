...
class Data_Labeling:
    ...
    def __call__(self, file_name, data_labeling_rules):
        self.Log(f'\n..:: Start Data_Labeling \"{file_name}\" by \"{data_labeling_rules}\" ::..')
        df = pd.read_csv(f'{self.iPath}/{file_name}.csv')

        self.create_outPutFolder(self.oPath)
        
        if("R2" in data_labeling_rules):
            df = self.calc_RCat2(df)
        if("B2" in data_labeling_rules):
            df = self.calc_BCat2(df)
        
        df.to_csv(f'{self.oPath}/{file_name}_{data_labeling_rules}.csv', index=False) 
        self.Log(f'..:: End Data_Labeling \"{file_name}\" by \"{data_labeling_rules}\" ::..')
